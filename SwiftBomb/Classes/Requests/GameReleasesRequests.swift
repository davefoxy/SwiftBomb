...
extension SwiftBomb {
    ...
    /**
     ...
     - parameter fields: An optional array of fields to return in the response. See the available options at http://www.giantbomb.com/api/documentation#toc-0-34. Pass nil to return everything.
     - parameter completion: A closure returning an optional generic `PaginatedResults` object containing the returned `GameReleaseResource` objects and pagination information and also, an optional `SwiftBombRequestError` object if the request failed.
     */
    public static func fetchReleases(_ query: String? = nil, pagination: PaginationDefinition? = nil, sort: SortDefinition? = nil, fields: [String]? = nil, completion: @escaping (PaginatedResults<GameReleaseResource>?, _ error: SwiftBombRequestError?) -> Void) {
        ...
        let request = requestFactory.gameReleasesRequest(query, pagination: pagination, sort: sort, fields: fields)
...
extension RequestFactory {
    ...
    func gameReleasesRequest(_ query: String? = nil, pagination: PaginationDefinition? = nil, sort: SortDefinition? = nil, fields: [String]? = nil) -> SwiftBombRequest {
        
        var request = SwiftBombRequest(configuration: configuration, path: "releases", method: .get, pagination: pagination, sort: sort, fields: fields)
...
extension GameReleaseResource {
    ...
    public func fetchExtendedInfo(_ fields: [String]? = nil, completion: @escaping (_ error: SwiftBombRequestError?) -> Void) {
        ...
        guard
            ...
            let id = id,
            let request = api.requestFactory?.simpleRequest("release/\(id)/", fields: fields) else {
